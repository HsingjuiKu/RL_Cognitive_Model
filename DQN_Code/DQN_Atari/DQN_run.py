...
def parse_args():
    ...
    parser.add_argument("--learning-starts", type=int, default=100, help="number of steps before learning starts")
    parser.add_argument("--learning-freq", type=int, default=1, help="number of iterations between every optimization step")
    parser.add_argument("--target-update-freq", type=int, default=300, help="number of iterations between every target network update")
...
class AgentRunner:
    ...
    def step(self, episode):
        ...
        min_q_value = np.percentile([np.min(values) for values in self.q_values_collection], 30)
        max_q_value = np.percentile([np.max(values) for values in self.q_values_collection], 70)
        ...
        min_reward = np.percentile([np.sum(values) for values in self.rewards_collection], 10)
        max_reward = np.percentile([np.sum(values) for values in self.rewards_collection], 90)
